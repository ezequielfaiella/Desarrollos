...
def edicionpedidos(ordendecompra):
                        ...
                        sucursal = lineas[0]
                        temporario.write("Tienda: "+sucursal[466:499]+" OC: "+ordendecompra+" | Fecha de entrega: "+sucursal[66:74])
                        separador = "_" * 70
                        # resumen.write("\n" + separador + "\n")
...
def resumen():
                    ...
                    if "totales.txt" in ordenesdecomprapendientes: ordenesdecomprapendientes.remove("totales.txt")
                    if "totales.txt" in ordenesdecomprapendientes: ordenesdecomprapendientes.remove("resumen.txt")
                    if "totales.txt" in ordenesdecomprapendientes: ordenesdecomprapendientes.remove("tiendas.txt")
                    for pendiente in ordenesdecomprapendientes:
                        if not pendiente.startswith('0'): continue
                        with open(pathpedido+pendiente, 'r') as file:
                            lineas = file.readlines()
                            total_lineas = len(lineas)
                            sucursal = lineas[0]
                            separador = "_" * 70
                            # print(lineas[int(total_lineas)])
                            total_pedido = lineas[-1].split(':')[1]
                            resumen.write(separador + "\n")
                            # resumen.write("Tienda: ""("+pendiente[0:4]+")"+sucursal[8:30]+"OC:"+pendiente[:-4]+" | Fecha de entrega: "+sucursal[82:91]+"\n")
                            resumen.write("Tienda: "+sucursal[8:30]+"OC:"+pendiente[:-4]+" | Fecha de entrega: "+sucursal[82:91]+"\n")
                            tiendas.write(pendiente[0:4]+" - "+sucursal[8:34].strip()+"-("+total_pedido+")"+"\n")
                            for n in range(1,int(total_lineas)-1):
                                articulo = lineas[n]
                                resumen.write("Item: "+articulo[6:34]+" | Cantidad: "+articulo[54:60]+" | EAN: "+articulo[103:117])
                    # return sucursales
                    ordenesdecomprapendientes = []
...
def haypedidoorecepcion():
    if total_pedidos != 0 or total_recepciones != 0:
        os.system('/bin/bash /media/trabajo/Trabajo/scripts/ingreso_pedidos.sh')
        
def pedidoconrecepcion():
    ordenesdecomprapendientes = []
    ...
    with os.scandir(pathpedido) as ficheros:
        for fichero in ficheros:
            if fichero.name.endswith(".txt"):
                if fichero.name.startswith("0"):
                    ordenesdecomprapendientes.append(fichero.name[0:-4] )

    # print(ordenesdecomprapendientes)

    with os.scandir(path) as recepciones:
        for recepcion in recepciones:
            if recepcion.name.startswith("CABCRE_"):
                with open(recepcion) as rece:
                    nrec = rece.readlines()
                    nrec = nrec[0][145:161]
                    # print(nrec)
                    if nrec in ordenesdecomprapendientes:
                        # print(nrec, ": el pedido existe, se borra")
                        os.remove(pathpedido+nrec+'.txt')
                        # print(pathpedido+nrec+'.txt')

def pedidocentro():
    pass
    if os.path.exists(path+'CABPED_0927*.TXT'):
        with os.scandir(pathpedido) as ficheros:
            for fichero in ficheros:
                if fichero.name.startswith("0927"):
                    contenido = fichero.read()
                    enviomensajetelegram(contenido)

def nuevas():
    pedidosnuevos = []
    with os.scandir(path) as ficheros:
        with open (pathpedido+"nuevas.txt", "w") as nuevosingresos:
            for fichero in ficheros:
                if fichero.name.startswith("CABPED_"):
                    pedidosnuevos.append(fichero.name[0:29])
            for pedido in pedidosnuevos:
                with open (path+pedido) as pedidoleido:
                    contenido = pedidoleido.readline()
                    sucursal = contenido[16:20]
                    nombre = contenido[466:501]
                    nuevosingresos.write("("+sucursal+")-"+nombre+"\n")
...
if __name__ == "__main__":
    ...
    if existe:
        ...
        if os.path.exists(pathpedido+'nuevas.txt'):
            os.remove(pathpedido+'nuevas.txt')
        ...
        with open("/media/trabajo/Trabajo/scripts/informe.log", 'w') as file:  
            ...
            file.write("\n" + str(separador) + "\n\n" )
            with open(pathpedido+'nuevas.txt', 'r', encoding='utf-8') as ingresos:
